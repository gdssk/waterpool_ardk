...
namespace Niantic.ARDK.Rendering.SRP
{
  public class ARSessionFeature: ScriptableRendererFeature
  {
    ...
    public void SetupBackgroundPass(Camera camera, CommandBuffer commandBuffer)
    {
      if (camera == null)
      {
        Debug.LogException(new ArgumentNullException("camera"));
        return;
      }

      if (commandBuffer == null)
      {
        Debug.LogException(new ArgumentNullException("commandBuffer"));
        return;
      }
    ...
    )
    {
      ...
      {
        if (_backgroundPass != null && renderingData.cameraData.camera == _backgroundPass.TargetCamera)
          renderer.EnqueuePass(_backgroundPass);
      }

      if (_afterRenderingPass != null && renderingData.cameraData.camera == _afterRenderingPass.TargetCamera)
