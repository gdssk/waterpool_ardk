...
namespace Niantic.ARDK.VirtualStudio.Remote
{
  ...
  internal static class _RemoteConnection
  {
    ...
    private static int _internalTargetFramerate;
    private static bool _overridenTargetFramerate;

    public static int TargetFramerate
    {
      get
      {
        if (_overridenTargetFramerate)
          return _internalTargetFramerate;

        if (_remoteConnectionImpl != null)
          return _remoteConnectionImpl.TargetFramerate;

        return 0;
      }
    }

    internal static void OverrideTargetFramerate(int newFramerate)
    {
      if (newFramerate == 0)
      {
        _overridenTargetFramerate = false;
        return;
      }

      _overridenTargetFramerate = true;
      _internalTargetFramerate = newFramerate;
    }
    ...
    private static void Dispose()
    {
      ...
      _RemoteDeviceARSessionConstructor.Deinitialize();
      _RemoteDeviceMultipeerNetworkingConstructor._Deinitialize();
      _RemoteDeviceARNetworkingConstructor.Deinitialize();
    }
    ...
    public static void Send(Guid id, byte[] data)
    {
      ...
      if (readyAndConnected)
      {
        ...
        _remoteConnectionImpl.Send(id, data);
      }
      else
      {
        var message =
    ...
    private static void TryClearBuffer()
    {
      ...
      foreach (var bufferedMessage in messages)
        Send(bufferedMessage.Tag, bufferedMessage.Data);
    }
    ...
    public static void Send<TValue>(Guid id, TValue value)
    {
      Send(id, value.SerializeToArray());
    }
