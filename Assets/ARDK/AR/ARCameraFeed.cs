...
namespace Niantic.ARDK.AR
{
    ...
    IDisposable
  {
    ...
    private static readonly Shader _brgaShader;
    private static readonly Shader _yuvShader;
    private static readonly Shader _brgaNonOESShader;
    ...
    static ARCameraFeed()
    {
      _brgaShader = Resources.Load<Shader>("BGRAShader");
      _yuvShader = Resources.Load<Shader>("YCbCrShader");
      _brgaNonOESShader = Resources.Load<Shader>("BGRANonOESShader");

      Assert.IsNotNull(_yuvShader, "_yuvShader != null");
      Assert.IsNotNull(_brgaShader, "_brgaShader != null");
      Assert.IsNotNull(_brgaNonOESShader, "_brgaNonOESShader != null");
    }
    ...
    )
    {
      ...
      {
#if NREAL_HEADSET
        _bgraMaterial = new Material(_brgaNonOESShader);
#else
        _bgraMaterial = new Material(_brgaShader);
#endif
    ...
    private void CorrectToActualResolution()
    {
      switch (_cameraFeedResolution.Mode)
      {
        ...
        case CameraFeedResolutionMode.FromHardware:
        {
          ...
          #if UNITY_STANDALONE_DESKTOP || UNITY_EDITOR
    ...
    private void GrabCameraFeed()
    {
      ...
      if (frame.CapturedImageTextures.Length == 0)
        return;
#endif
