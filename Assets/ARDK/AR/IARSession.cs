...
namespace Niantic.ARDK.AR
{
    ...
    IDisposable
  {
    ...
    /// The mesh most recently updated by the session.
    /// @note **May be null**.
    // IARMesh CurrentMesh { get; }
    ...
    /// @param worldScale
    ///   The Unity-specific scaling factor applied to this anchor.
    ...
    /// Set up this session to listen to updates from the provided location service. This is
    /// required when running the session with a MappingRole configuration value other
    /// than MapperIfHost.
    ...
    /// Called when the session receives an updated Mesh.
    event ArdkEventHandler<MeshUpdatedArgs> MeshUpdated;
