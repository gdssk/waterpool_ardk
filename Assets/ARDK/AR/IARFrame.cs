...
namespace Niantic.ARDK.AR
{
    ...
    IDisposable
  {
    ...
    /// @note **May be null**
    /// </summary>
    IARPointCloud DepthFeaturePoints { get; }
    ...
    // TODO: Review if it is compressionLevel or compressionQuality (100% for compression means
    // best compression, while 100% for quality means worst compression).
    /// <summary>
    /// Get a serialized representation of this ARFrame, with variable compression levels.
    /// Defaults to a compression factor of 70%.
    /// Will only generate the image buffers associated with the frame at a limited frame rate
    ///   (See RemoteConnection.TargetFramerate)
    /// </summary>
    /// <param name="compressionLevel">The level to which the image buffer is compressed (0-100%)</param>
    IARFrame LazySerialize(int compressionLevel = 70);
    ...
    /// </summary>
    /// <param name="compressionLevel">The level to which the image buffer is compressed (0-100%)</param>
    IARFrame Serialize(int compressionLevel = 70);
  }
}
