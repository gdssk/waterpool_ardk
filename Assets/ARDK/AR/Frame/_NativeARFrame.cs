...
#pragma warning restore 0162
    ...
    internal _NativeARFrame(IntPtr nativeHandle, float worldScale)
    {
      if (nativeHandle == IntPtr.Zero)
        throw new ArgumentException("nativeHandle can't be Zero.", "nativeHandle");
    ...
    public _NativeARCamera Camera
    {
      get
      {
        ...
        if (result == null)
        {
          ...
          result = _NativeARCamera._FromNativeHandle(cameraHandle, WorldScale);
          _camera = result;
        }
...
#pragma warning restore 0162
    ...
    public IARFrame LazySerialize(int compressionLevel = 70)
    {
      _threadChecker._CheckThread();

      var captureImages = false;

      if (_RemoteConnection.TargetFramerate > 0)
      {
        var delta = Time.unscaledTime - _lastCaptureTime;
        var targetDelta = 1.0f / _RemoteConnection.TargetFramerate;
      ...
      var serialized = captureImages
        ? _SerializeWithCompression(compressionLevel)
        : _SerializeWithoutBuffers();

      return serialized;
    }

    public IARFrame Serialize(int compressionLevel = 70)
    {
      return _SerializeWithCompression(compressionLevel);
    }

    private _SerializableARFrame _SerializeWithoutBuffers()
    {
      _threadChecker._CheckThread();
    ...
    private IARFrame _SerializeWithCompression(int compressionLevel)
    {
      _threadChecker._CheckThread();

      var serializedFrame = _SerializeWithoutBuffers();
