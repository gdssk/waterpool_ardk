...
namespace Niantic.ARDK.AR.Configuration
{
  internal static class _ARConfigurationValidator
  {
    ...
    )
    {
      ...
      var isEditorSession = session.ARInfoSource != ARInfoSource.LiveDevice;
    ...
    private static void SetMissingValues(this IARConfiguration config)
    {
      ...
      var isDepthEnabled = (worldConfig.DepthFeatures & DepthFeatures.Depth) != 0;
      var isPointCloudEnabled = (worldConfig.DepthFeatures & DepthFeatures.PointCloud) != 0;

      if (isPointCloudEnabled && !isDepthEnabled)
      {
        ARLog._Debug
        (
          "Enabling DepthFeatures.Depth because DepthFeatures.PointCloud was enabled."
        );

        worldConfig.DepthFeatures |= DepthFeatures.Depth;
      }
