...
namespace Niantic.ARDK.AR.Networking
{
    ...
    IARNetworking
  {
    ...
    public IReadOnlyDictionary<IPeer, IARPeerAnchor> PeerAnchors
    {
      get
      {
        return _networkAnchorManager.PeerAnchors;
      }
    }
    ...
    public IReadOnlyDictionary<SharedAnchorIdentifier, IARSharedAnchor> SharedAnchors
    {
      get
      {
        return _networkAnchorManager.SharedAnchors;
      }
    }
...
#pragma warning restore 0162
    ...
    public bool CreateSharedAnchor(Matrix4x4 transform,  out SharedAnchorIdentifier identifier)
    {
      ...
      identifier = new SharedAnchorIdentifier();

      if (LocalPeerState != PeerState.Stable)
      {
        ARLog._Warn("Cannot create shared anchors until the LocalPeerState has reached Stable.");
        return false;
      }
      ...
      var requestSendResult = 0;
      if (NativeAccess.Mode == NativeAccess.ModeType.Native)
      {
        _NARARMultipeerNetworking_CreateSharedAnchor
        (
          _nativeHandle,
          guid,
          nativeTransform
      ...
      if (requestSendResult == 0)
      {
        ARLog._DebugFormat
        (
          "Failed to send a request creating shared anchor: {0}",
          false,
          guid
        );
        return false;
      ...
      // Fill in the new anchor's data preemptively, with a status that makes it clear it's not
      // currently being tracked.

      identifier = new SharedAnchorIdentifier(guid);
      var anchorData =
        new _SharedAnchorDataUpdate
        (
          identifier,
          SharedAnchorTrackingState.Unresolved,
          null
        );

      _networkAnchorManager.AddSharedAnchor(anchorData);

      ARLog._DebugFormat("Created a shared anchor: {0}", false, guid);
    ...
    private void SubscribeToInternalCallbacks(bool force = false)
    {
      ...
      if (force)
      {
        ...
        _sharedAnchorsUpdateInitialized = false;
        _sharedAnchorsUploadedInitialized = false;
        _peerAnchorUpdateInitialized = false;
      }

      SubscribeToDidReceivePoseFromPeer();
      SubscribeToDidReceiveStateFromPeer();

      if (_networkAnchorManager.SharedAnchorsUpdated != null)
        SubscribeToSharedAnchorsUpdate();

      if (_networkAnchorManager.SharedAnchorsUploaded != null)
        SubscribeToSharedAnchorsUploaded();

      if (_networkAnchorManager.PeerAnchorUpdated != null)
        SubscribeToPeerAnchorUpdate();
    }
...
#region SharedAnchorsTrackingUpdated
    private bool _sharedAnchorsUpdateInitialized = false;

    private unsafe void SubscribeToSharedAnchorsUpdate()
    {
      _CheckThread();

      if (_sharedAnchorsUpdateInitialized)
        return;

      if (NativeAccess.Mode == NativeAccess.ModeType.Native)
      {
        _NARARMultipeerNetworking_Set_sharedAnchorsUpdateCallback
        (
          _applicationHandle,
          _nativeHandle,
          _onSharedAnchorsUpdateNative
        );
      }

      _sharedAnchorsUpdateInitialized = true;
    }
    ...
    public event ArdkEventHandler<SharedAnchorsArgs> SharedAnchorsTrackingUpdated
    {
      ...
      {
        SubscribeToSharedAnchorsUpdate();
        _networkAnchorManager.SharedAnchorsUpdated += value;
      }
      remove
      {
        _networkAnchorManager.SharedAnchorsUpdated -= value;
    ...
    // Use of pointers is just for unpacking the SimpleArray passed from C++. No unmanaged
    // memory going on here.
    [StructLayout((LayoutKind.Sequential))]
    private unsafe struct ArrayOfSharedAnchorInfo
    {
      public SharedAnchorInfo* array;
      public UInt64 arraySize;
    }

    [StructLayout(LayoutKind.Sequential)]
    private struct SharedAnchorInfo
    {
      public Guid identifier;
      public IntPtr anchorToLocalTransform;
      public Int64 status;
    }

    [MonoPInvokeCallback(typeof(_NARARMultipeerNetworking_Shared_Anchors_Update_CallbackDelegate))]
    private static unsafe void _onSharedAnchorsUpdateNative
    (
       IntPtr context,
       ArrayOfSharedAnchorInfo* infoPtrs
    )
    {
      var arNetworking = SafeGCHandle.TryGetInstance<_NativeARNetworking>(context);
      if (arNetworking == null)
      ...
      ARLog._Debug("Batch updating shared anchors", true);

      SharedAnchorInfo* anchorsInfoArray = infoPtrs->array;
      var anchorsCount = infoPtrs->arraySize;

      for (var i = 0; i < (int) anchorsCount; i++)
      {
        var info = anchorsInfoArray[i];

        Matrix4x4? anchorToLocalMatrix = null;

        var trackingState = (SharedAnchorTrackingState) info.status;
        if (trackingState == SharedAnchorTrackingState.Tracking)
        {
          // Minor optimization by not doing the conversion unless needed
          var anchorToLocalTransformSchema = new float[16];
          Marshal.Copy(info.anchorToLocalTransform, anchorToLocalTransformSchema, 0, 16);

          var tempMatrix = _Convert.InternalToMatrix4x4(anchorToLocalTransformSchema);
          anchorToLocalMatrix = NARConversions.FromNARToUnity(tempMatrix);
        }

        var anchorData =
          new _SharedAnchorDataUpdate
          (
            new SharedAnchorIdentifier((Guid) info.identifier),
            trackingState,
            anchorToLocalMatrix
          );

        arNetworking._networkAnchorManager.QueueSharedAnchor(anchorData);
      }
    }
#endregion

#region SharedAnchorsUploaded
    private bool _sharedAnchorsUploadedInitialized = false;

    private unsafe void SubscribeToSharedAnchorsUploaded()
    {
      _CheckThread();

      if (_sharedAnchorsUploadedInitialized)
        return;

      if (NativeAccess.Mode == NativeAccess.ModeType.Native)
      {
        _NARARMultipeerNetworking_Set_sharedAnchorsPersistedCallback
        (
          _applicationHandle,
          _nativeHandle,
          _onSharedAnchorsUploadedNative
        );
      }

      _sharedAnchorsUploadedInitialized = true;
    }
    ...
    public event ArdkEventHandler<SharedAnchorsArgs> SharedAnchorsUploaded
    {
      ...
      {
        SubscribeToSharedAnchorsUploaded();
        _networkAnchorManager.SharedAnchorsUploaded += value;
      }
      remove
      {
        _networkAnchorManager.SharedAnchorsUploaded -= value;
    ...
    [StructLayout(LayoutKind.Sequential)]
    private unsafe struct ArrayOfAnchorIdentifiers
    {
      public Guid* array;
      public UInt64 arraySize;
    }

    [MonoPInvokeCallback(typeof(_NARARMultipeerNetworking_Shared_Anchors_Persisted_CallbackDelegate))]
    private static unsafe void _onSharedAnchorsUploadedNative
    (
      IntPtr context,
      ArrayOfAnchorIdentifiers* uuidPtrs
    )
    {
      var arNetworking = SafeGCHandle.TryGetInstance<_NativeARNetworking>(context);
      if (arNetworking == null)
      ...
      ARLog._Debug("Batch uploading shared anchors", true);

      Guid* uuidArray = uuidPtrs->array;
      var anchorsCount = uuidPtrs->arraySize;

      for (var i = 0; i < (int) anchorsCount; i++)
      {
        var identifier = new SharedAnchorIdentifier((Guid) uuidArray[i]);
        arNetworking._networkAnchorManager.QueueUploadedAnchor(identifier);
      }
    }
#endregion

#region PeerAnchorUpdated
    private bool _peerAnchorUpdateInitialized = false;

    private unsafe void SubscribeToPeerAnchorUpdate()
    {
      _CheckThread();

      // Also subscribe to peer pose and state updates as they'll be synthesized into the peer anchor.
      SubscribeToDidReceivePoseFromPeer();
      SubscribeToDidReceiveStateFromPeer();

      if (_peerAnchorUpdateInitialized)
        return;

      if (NativeAccess.Mode == NativeAccess.ModeType.Native)
      {
        _NARARMultipeerNetworking_Set_peerAnchorsUpdateCallback
        (
          _applicationHandle,
          _nativeHandle,
          _onPeerAnchorUpdateNative
        );
      }

      _peerAnchorUpdateInitialized = true;
    }
    ...
    public event ArdkEventHandler<PeerAnchorUpdatedArgs> PeerAnchorUpdated
    {
      ...
      {
        SubscribeToPeerAnchorUpdate();
        _networkAnchorManager.PeerAnchorUpdated += value;
      }
      remove
      {
        _networkAnchorManager.PeerAnchorUpdated -= value;
    ...
    // Use of pointers is just for unpacking the SimpleArray passed from C++. Memory allocated for
    // the peer is handled through _NativeMultipeerNetworking.GetPeerFromUnmanagedHelper(handle).
    [StructLayout((LayoutKind.Sequential))]
    private unsafe struct ArrayOfPeerAnchorInfo
    {
      public PeerAnchorInfo* array;
      public UInt64 arraySize;
    }

    [StructLayout(LayoutKind.Sequential)]
    private struct PeerAnchorInfo
    {
    ...
    [MonoPInvokeCallback(typeof(_NARARMultipeerNetworking_Peer_Anchors_Update_CallbackDelegate))]
    private static unsafe void _onPeerAnchorUpdateNative
    (
       IntPtr context,
       ArrayOfPeerAnchorInfo* peerPtrs
    )
    {
      ARLog._Debug("Batch updating peer anchors", true);

      int peerCount = (int)peerPtrs->arraySize;
      var peerInfoArray = peerPtrs->array;

      var arNetworking = SafeGCHandle.TryGetInstance<_NativeARNetworking>(context);
      if (arNetworking == null || arNetworking.IsDestroyed)
      {
        for (var i = 0; i < peerCount; i++)
          _Peer._ReleasePeer(peerInfoArray[i].handle);

        return;
      }

      var peerInfos = new PeerAnchorInfo[peerCount];
      for (var i = 0; i < peerCount; i++)
        peerInfos[i] = peerInfoArray[i];

      // _GetPeerFromUnmanagedHelper can only be called from the main thread.
      _CallbackQueue.QueueCallback
      (
        () =>
        {
          ...
          {
            for (var i = 0; i < peerCount; i++)
              _Peer._ReleasePeer(peerInfos[i].handle);
          ...
          for (var i = 0; i < peerCount; i++)
          {
            ...
            var peer = _Peer.FromNativeHandle(peerInfo.handle);

            Marshal.Copy(peerInfo.anchorToLocalTransform, anchorToLocalTransformSchema, 0, 16);
            var anchorToLocalMatrix =
              NARConversions.FromNARToUnity(_Convert.InternalToMatrix4x4(anchorToLocalTransformSchema));

            var peerData =
              new PeerAnchorData
              (
                peer,
                (PeerAnchorStatus) peerInfo.status,
                anchorToLocalMatrix
              );

            if (networkAnchorManager != null)
              networkAnchorManager.QueuePeerAnchor(peerData);
          }
        }
      );
    }
    ...
    [DllImport(_ARDKLibrary.libraryName)]
    private static extern void _NARARMultipeerNetworking_Set_sharedAnchorsUpdateCallback
    (
      IntPtr context,
      IntPtr nativeHandle,
      _NARARMultipeerNetworking_Shared_Anchors_Update_CallbackDelegate cb
    );

    [DllImport(_ARDKLibrary.libraryName)]
    private static extern void _NARARMultipeerNetworking_Set_sharedAnchorsPersistedCallback
    (
      IntPtr context,
      IntPtr nativeHandle,
      _NARARMultipeerNetworking_Shared_Anchors_Persisted_CallbackDelegate cb
    );

    [DllImport(_ARDKLibrary.libraryName)]
    private static extern void _NARARMultipeerNetworking_Set_peerAnchorsUpdateCallback
    (
      IntPtr context,
      IntPtr nativeHandle,
      _NARARMultipeerNetworking_Peer_Anchors_Update_CallbackDelegate cb
    );

    [DllImport(_ARDKLibrary.libraryName)]
    private static extern UInt32 _NARARMultipeerNetworking_CreateSharedAnchor
    (
      IntPtr nativeHandle,
      Guid newAnchorId,
      float[] newAnchorTransform
    );
    ...
    private unsafe delegate void _NARARMultipeerNetworking_Shared_Anchors_Update_CallbackDelegate
    (
       IntPtr context,
       ArrayOfSharedAnchorInfo* anchorInfos
    );

    private unsafe delegate void _NARARMultipeerNetworking_Peer_Anchors_Update_CallbackDelegate
    (
       IntPtr context,
       ArrayOfPeerAnchorInfo* peerInfos
    );

    private unsafe delegate void _NARARMultipeerNetworking_Shared_Anchors_Persisted_CallbackDelegate
    (
      IntPtr context,
      ArrayOfAnchorIdentifiers* uuid
    );
  }
}
