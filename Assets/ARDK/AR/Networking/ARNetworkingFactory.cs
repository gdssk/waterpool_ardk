...
using Niantic.ARDK.VirtualStudio.AR.Networking.Remote;
...
namespace Niantic.ARDK.AR.Networking
{
  ...
  public static class ARNetworkingFactory
  {
    ...
    /// Create an ARNetworking with the specified ARInfoSource.
    ///
    /// This will also create an ARSession object. If an ARSession object has already been created,
    ///   use the ARNetworkingFactory.Create(IARSession session) API instead.
    ///
    /// @param source
    ///   The source used to create the ARNetworking for.
    /// @param stageIdentifier
    ///   The identifier used by the C++ library to connect all related components.
    ///
    /// @returns The created ARNetworking, or throws if it was not possible to create a session.
    public static IARNetworking Create(ARInfoSource source)
    {
      return _Create(new List<ARInfoSource>(){source});
    }
    ...
    public static IARNetworking Create(IARSession session, ServerConfiguration configuration)
    {
      ...
      var source = session.ARInfoSource;

      var networking = MultipeerNetworkingFactory.Create(source, configuration, session.StageIdentifier);
    ...
    public static IARNetworking Create(IARSession session, IMultipeerNetworking networking)
    {
      ...
      var source = session.ARInfoSource;
      if (source != networking.ARInfoSource)
        throw new ArgumentException("session and networking must have the same ARInfoSource.");
      ...
      switch (source)
      {
        ...
        default:
          throw new InvalidEnumArgumentException("session.ARInfoSource", (int)source, source.GetType());
      }
    ...
    /// Tries to create an ARNetworking of any of the given sources.
    ///
    /// @param sources
    ...
    internal static IARNetworking _Create(IEnumerable<ARInfoSource> sources = null)
    {
      IARSession session;
      try
      {
        session = ARSessionFactory._Create(sources);
      }
    ...
    {
      var source = arSession.ARInfoSource;
      if (source != networking.ARInfoSource)
        throw new ArgumentException("arSession and networking must have the same ARInfoSource.");
      ...
      switch (source)
      {
        ...
        default:
          throw new InvalidOperationException("Invalid or unknown ARInfoSource: " + source);
      }
