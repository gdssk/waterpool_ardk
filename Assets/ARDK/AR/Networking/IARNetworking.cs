...
namespace Niantic.ARDK.AR.Networking
{
    ...
    IDisposable
  {
    ...
    /// Peer anchors for each peer (including the local peer).
    /// @note This is currently in internal development, and not useable.
    IReadOnlyDictionary<IPeer, IARPeerAnchor> PeerAnchors { get; }

    /// All the shared anchors available on the active map layer.
    /// @note This is currently in internal development, and not useable.
    IReadOnlyDictionary<SharedAnchorIdentifier, IARSharedAnchor> SharedAnchors { get; }
    ...
    /// Create an anchor that will be uploaded to this session's map layer in the cloud. Once
    /// this anchor is shared (see IARNetworking.SharedAnchorsUploaded), it is discoverable
    /// by other sessions on the same map layer.
    /// @param[in] transform
    ///   Position and rotation of the anchor in the coordinate space of the AR session
    /// @param[out] identifier Identifier of the shared anchor created, if creation was successful
    /// @returns bool True if creation was successful.
    /// @note This is currently in internal development, and not useable.
    bool CreateSharedAnchor(Matrix4x4 transform, out SharedAnchorIdentifier identifier);
    ...
    /// Alerts subscribers when a peer's (including the local peer's) anchor is updated.
    /// @note This is currently in internal development, and not useable.
    event ArdkEventHandler<PeerAnchorUpdatedArgs> PeerAnchorUpdated;

    /// Alerts subscribers when shared anchors are updated. Every shared anchor created locally
    /// or downloaded from the cloud will get surfaced through this event at least once.
    /// @note This is currently in internal development, and not useable.
    event ArdkEventHandler<SharedAnchorsArgs> SharedAnchorsTrackingUpdated;

    /// Alerts subscribers when shared anchors are successfully saved to the cloud.
    /// @note This is currently in internal development, and not useable.
    event ArdkEventHandler<SharedAnchorsArgs> SharedAnchorsUploaded;
