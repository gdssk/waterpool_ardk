...
namespace Niantic.ARDK.Extensions
{
  ...
  ///
  /// @note
  ///   Compatible with all ARInfoSources, where image data is provided in different ways.
  public class ARCameraRenderingHelper: MonoBehaviour
  {
    private const string ARDKMockLayerName = "ARDK_MockWorld";
    ...
    private void SetupRendering(Vector2 scale, Vector2 offset)
    {
      ...
      // If it is a mock session, attempt to disable the "ARDK_MockWorld" layer
      if (_arSession.ARInfoSource == ARInfoSource.Mock)
      {
        var layerMask = LayerMask.NameToLayer(ARDKMockLayerName);
        if (layerMask != -1)
        {
        ...
        {
          ARLog._Error("Add the ARDK_MockWorld layer to your project to run Mock AR sessions");
        }
      }
      ...
      var clearDepth =
       ...
       worldConfig.DepthFeatures != DepthFeatures.None;
